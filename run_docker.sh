...
nvidia-docker run --name strudl_sess -p 80:80 -v $PWD:/code/ -v $DATAPATH:/data/ --mount type=bind,src=/media,dst=/usb,bind-propagation=rslave -it strudl python3 server.py
